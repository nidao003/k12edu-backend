...
CREATE TABLE IF NOT EXISTS sync_events (
 ...
 payload JSONB NOT NULL, client_created_at TIMESTAMPTZ NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), archived_at TIMESTAMPTZ
);
...
ALTER TABLE sync_events ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;
ALTER TABLE sync_events ADD COLUMN IF NOT EXISTS field_timestamps JSONB NOT NULL DEFAULT '{}'::jsonb;
...
CREATE TABLE IF NOT EXISTS ai_policies (
 id UUID PRIMARY KEY, user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
 period TEXT NOT NULL, request_count INT NOT NULL DEFAULT 0, input_tokens INT NOT NULL DEFAULT 0,
 output_tokens INT NOT NULL DEFAULT 0, cost_micros BIGINT NOT NULL DEFAULT 0,
 updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(user_id, period)
);
...
CREATE TABLE IF NOT EXISTS ai_safety_events (
 id UUID PRIMARY KEY, user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
 reason TEXT NOT NULL, content_hash TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'open', reviewer_id UUID REFERENCES users(id), decision TEXT NOT NULL DEFAULT '', reviewed_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
ALTER TABLE ai_safety_events ADD COLUMN IF NOT EXISTS severity TEXT NOT NULL DEFAULT 'medium';
ALTER TABLE ai_safety_events ADD COLUMN IF NOT EXISTS appeal_status TEXT NOT NULL DEFAULT '';
...
CREATE TABLE IF NOT EXISTS user_files (id UUID PRIMARY KEY,user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,storage_key TEXT NOT NULL UNIQUE,name TEXT NOT NULL,size_bytes BIGINT NOT NULL,content_type TEXT NOT NULL DEFAULT 'application/octet-stream',safety_status TEXT NOT NULL DEFAULT 'pending',created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
ALTER TABLE user_files ADD COLUMN IF NOT EXISTS safety_status TEXT NOT NULL DEFAULT 'pending';
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS minor_mode BOOLEAN NOT NULL DEFAULT FALSE;
`
